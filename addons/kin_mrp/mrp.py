...
class MrpProduction(models.Model):
    ...
    def write(self, vals, update=True, mini=True):
        ...
        if all([l.state  == 'done' for l in self.batch_id.manufacturing_ids]):
            self.batch_id.state = 'done'
        elif any([l.state  == 'cancel' for l in self.batch_id.manufacturing_ids]):
            self.batch_id.state = 'cancel'
        elif any([l.state  in ('confirmed','ready','in_production') for l in self.batch_id.manufacturing_ids]):
            self.batch_id.state = 'progress'
        elif all([l.state  == 'draft' for l in self.batch_id.manufacturing_ids]):
