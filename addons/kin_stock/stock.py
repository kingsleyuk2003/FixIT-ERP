...
# Load all unsold PO lines
    ...
    #             purchase_lines[move.product_id.id] = move.purchase_line_id
    #             sales_lines[move.product_id.id] = move.sale_line_id
    ...
    #             purchase_line_id = purchase_lines[key[0]].id
    #             sale_line_id = sales_lines[key[0]].id
...
# even the account merge addon merges invoice perfectly after creating invoice as a result of backorder operation with the syst
    def create_customer_invoice(self):
        ...
        for picking_line in self.pack_operation_ids:
            sale_order_line_id = picking_line.sale_line_id
...
# even the account merge addon merges invoice perfectly after creating invoice as a result of backorder operation with the syst
    def create_vendor_bill(self):
        ...
        if partner :
            ...
            lines = []
            for picking_line in self.pack_operation_ids:
                purchase_order_line_id = picking_line.purchase_line_id
                taxes = picking_line.purchase_line_id.taxes_id
                invoice_line_tax_ids = picking_line.purchase_line_id.order_id.fiscal_position_id.map_tax(taxes)
            ...
            if not purchase_inv.invoice_line_ids:
                raise UserError(_('There is no invoiceable line.'))
            # If invoice is negative, do a refund invoice instead
            if purchase_inv.amount_untaxed < 0:
    ...
    def do_transfer(self):
        ...
        if is_create_invoice :
            ...
            if picking_type_code == "incoming" and sale_order and invoic_obj : # Create a Customer refund invoice
                ...
                invoice_refund = self.env['account.invoice.refund'] #a refund wizard
                ...
                if is_send_invoice_notification and company_email :
                    ...
                    for user in users :
                        if user.has_group('kin_stock.group_receive_inventory_invoice_email_refund') and user.partner_id.email and user.partner_id.email.strip() :
            ...
            if picking_type_code == "outgoing" and sale_order    : # Create a Customer Invoice
                ...
                if inv_id : # if inv_id is empty , maybe as a result of an already created invoice, thus no invoice may be created here. This can happen when sales order is cancelled and open again with another delivery that creates an invoice
                    ...
                    if is_send_invoice_notification and company_email :
                        ...
                        for user in users :
                            if user.has_group('kin_stock.group_receive_inventory_invoice_email') and user.partner_id.email and user.partner_id.email.strip() :
                                ctx = {'system_email': company_email,
                                       'accountant_email':user.partner_id.email,
                                       'accountant_name': user.partner_id.name ,
                                       'url':the_url,
        ...
        ######   FOR PURCHASES VENDOR BILLS AND PURCHASE RETURNS
            ...
            if prev_picking_obj :
                ...
                if picking_type_code == "outgoing" and prev_purchase_order and prev_invoice_id  : # Create a Return goods Invoice
                    ...
                    pur_invoice_refund = self.env['account.invoice.refund']
                    pur_invoice_refund_obj= pur_invoice_refund.create({'description':' Inventory Return to Vendor for %s'% self.name,'filter_refund':'refund'})
                    prev_ctx = {'active_ids': [prev_invoice_id.id]}
                    pur_invoice_refund_obj.env.context = prev_ctx
                    pur_return_obj = pur_invoice_refund_obj.compute_refund()
                    pur_invo_id = pur_return_obj['domain'][1][2][0]
                    pur_invref_obj = invoice_obj.browse(pur_invo_id)
                    ...
                    if company_email :
                        ...
                        for user in users :
                            if user.has_group('kin_stock.group_receive_purchase_bill_email_refund') and user.partner_id.email and user.partner_id.email.strip() :
                                ctx = {'system_email': company_email,
                                        'accountant_email':user.partner_id.email,
                                        'accountant_name': user.partner_id.name ,
                                        'url':the_url,
...
class StockReturnPicking(models.TransientModel):
    ...
    def default_get(self, cr, uid, fields, context=None):
        ...
        if pick:
            if pick.state not in ['done','shipped','delivered','returned']:
                raise UserError(_("You may only return pickings that are Done, Shipped, Delivered, Returned States !"))
...
class StockMove(models.Model):
    ...
    def _picking_assign(self, cr, uid, move_ids, context=None):
        ...
        if picks:
            ...
            pick_obj.message_post(cr, uid, pick, body=_('Transfer Document Created -  %s.') % (pick_name),
                                  subject='Please see the Newly Created Transfer Picking Document',
    ...
    def onchange_product_id(self, prod_id=False, loc_id=False, loc_dest_id=False, partner_id=False):
        res = super(StockMove,self).onchange_product_id(prod_id=prod_id, loc_id=loc_id, loc_dest_id=loc_dest_id, partner_id=partner_id)
...
class ProcurementOrderExtend(models.Model):
    ...
    #NO NEED AGAIN. sale_line_id is an internal field already existing in odoo
    # @api.model
    # def _run_move_create(self, procurement):
    #     vals = super(ProcurementOrderExtend, self)._run_move_create(procurement)
    #     if procurement.sale_line_id:
    #         vals.update({'sale_line_id': procurement.sale_line_id.id})
    #     return vals
